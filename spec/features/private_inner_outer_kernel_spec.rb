...
describe 'Symbiont: inner context (object) => outer context (proc) => kernel context (kernel)' do
  ...
  specify 'private IOK resolution' do
    ...
    result = Symbiont::Executor.evaluate_private(object, Symbiont::IOK, &closure)
    method = Symbiont::Executor.private_method(:object_data, object, Symbiont::IOK, &closure)
    ...
    expect do
      Symbiont::Executor.evaluate_private(object, Symbiont::IOK, &closure)
    end.to raise_error(Symbiont::Trigger::ContextNoMethodError)

    expect do
      Symbiont::Executor.private_method(:object_data, object, Symbiont::IOK, &closure)
    end.to raise_error(Symbiont::Trigger::ContextNoMethodError)
  end
end
