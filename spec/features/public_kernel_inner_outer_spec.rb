...
describe 'Symbiont: kernel context (kernel) => inner context (object) => outer context (proc)' do
  ...
  specify 'public KIO resolution' do
    ...
    result = Symbiont::Executor.evaluate(object, Symbiont::KIO, &closure)
    method = Symbiont::Executor.public_method(:object_data, object, Symbiont::KIO, &closure)
    ...
    undef object_data
    expect do
      Symbiont::Executor.evaluate(object, Symbiont::KIO, &closure)
    end.to raise_error(Symbiont::Trigger::ContextNoMethodError)

    expect do
      Symbiont::Executor.public_method(:object_data, object, Symbiont::KIO, &closure)
    end.to raise_error(Symbiont::Trigger::ContextNoMethodError)
  end
end
