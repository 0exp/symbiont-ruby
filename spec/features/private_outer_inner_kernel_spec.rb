...
describe 'Symbiont: outer context (proc) => inner context (object) => kernel context (kernel)' do
  ...
  specify 'private OIK resolution' do
    ...
    result = Symbiont::Executor.evaluate_private(object, Symbiont::OIK, &closure)
    method = Symbiont::Executor.private_method(:object_data, object, Symbiont::OIK, &closure)
    ...
    expect do
      Symbiont::Executor.evaluate_private(object, Symbiont::OIK, &closure)
    end.to raise_error(Symbiont::Trigger::ContextNoMethodError)

    expect do
      Symbiont::Executor.private_method(:object_data, object, Symbiont::OIK, &closure)
    end.to raise_error(Symbiont::Trigger::ContextNoMethodError)
  end
end
